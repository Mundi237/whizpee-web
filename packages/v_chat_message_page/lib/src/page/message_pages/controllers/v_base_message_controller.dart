...
abstract class VBaseMessageController extends MessageStateController with StreamMix {
  ...
  void onGetClipboardImageBytes(Uint8List imageBytes) async {
    final fileRes = await context.toPage(VMediaEditorView(
      files: [VPlatformFile.fromBytes(bytes: imageBytes.toList(), name: "${uuid.v4()}.png")],
      config: VMediaEditorConfig(
        imageQuality: vMessageConfig.compressImageQuality,
      ),
    )) as List<VBaseMediaRes>?;
  ...
  void _initReactionController() {
    // ReactionController uses static methods, no instance needed
  }
